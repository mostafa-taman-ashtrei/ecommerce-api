...
import { DELETE_ACTION, RESTORE_ACTION } from '../constants';
import { addProduct, deleteAndRestoreProductById, getAllProducts, getProductById } from '../services/product.service';
...
export const handleDeltingProducts = async (req: Request, res: Response) => {
    ...
    if (String(action) !== DELETE_ACTION && String(action) !== RESTORE_ACTION) return res.status(400).json({ e: 'Invalid action' });
    if (String(id).length !== 24) return res.status(400).json({ e: 'Invalid Id' });
