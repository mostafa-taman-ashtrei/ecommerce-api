...
import { addProduct, deleteProductById, getAllProducts, getProductById, restoreProductById } from '../services/product.service';
...
export const handleDeltingProducts = async (req: Request, res: Response) => {
    ...
    const response = await deleteProductById(String(id));

    if (response.e) return res.status(response.status).json({ e: response.e });
    return res.status(response.status).json({ data: response.data });
};

export const handleRestoringProducts = async (req: Request, res: Response) => {
    const { id } = req.params;

    if (String(id).length !== 24) return res.status(400).json({ e: 'Invalid Id' });

    const response = await restoreProductById(String(id));
